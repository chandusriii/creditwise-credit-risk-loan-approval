...
def train_creditwise_model():
    ...
    # The CSV uses Yes/No. Normalize text before mapping so capitalization or
    # accidental whitespace cannot turn every target into NaN.
    ...
    if valid.sum() == 0:
        raise ValueError(
            "No valid Loan_Approved labels were found. Expected values such as Yes/No."
        )
    ...
    # Match the documented feature engineering in the project.
    ...
    # Learn the education ordering from the categories documented/present in
    # this dataset, while safely handling unseen values at inference time.
...
# Train once per Streamlit process.
try:
    df = load_data()
...
st.info(
    "Enter applicant details to generate an Approved/Rejected prediction. "
    "The prediction supports loan officers before final human verification."
)


def options(column):
    """Return clean, non-empty categories from the actual training CSV."""
...
with st.form("creditwise_form"):
    ...
    loan_terms = sorted(
        pd.to_numeric(df["Loan_Term"], errors="coerce").dropna().unique().tolist()
    )
    if not loan_terms:
    ...
    submitted = st.form_submit_button(
        "🔮 Predict Loan Approval", use_container_width=True
    )
...
if submitted and model is not None:
    ...
    # Match training-time feature engineering exactly.
    ...
    st.progress(
        probability,
        text=f"Estimated approval probability: {probability * 100:.1f}%",
    )

    st.caption(
        "Educational demonstration. The deployed model is trained on the historical "
        "dataset for inference. The project's reported 88.00% accuracy, 83.61% recall "
        "and 80.95% F1-score are from the documented held-out test evaluation. "
        "Final lending decisions require human verification."
    )
